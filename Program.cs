...
class Programm
{
    ...
    async static Task Main()
    {
        ...
        GitHubReposDb apiDb = new GitHubReposDb(settingsService.CurrentSettings.GitHubReposDatabase);
        apiDb.Database.EnsureCreated();

        ScrapingDb documentDb = new ScrapingDb(settingsService.CurrentSettings.ScrapingDatabase);
        ...
        while (true)
        {
            ...
            if (choice == "1")
            {
                await GitHubMenu(githubService, apiDb);
            }
            else if (choice == "2")
            {
                await ScrapingMenu(webScraper, documentDb);
            }
            else if (choice == "3")
            {
        ...
        async Task GitHubMenu(GitHubService service, GitHubReposDb db)
        {
            while (true)
            {
                Console.Clear();
        ...
        async Task ScrapingMenu(WebScraper scraper, ScrapingDb db)
        {
            while (true)
            {
                Console.Clear();
        ...
        async Task SearchRepositories(GitHubService service, GitHubReposDb db)
        {
            Console.Clear();
        ...
        void ShowSavedRepositories(GitHubReposDb db)
        {
            while (true)
            {
                Console.Clear();
        ...
        async Task ScrapeUrl(WebScraper scraper, ScrapingDb db)
        {
            Console.Clear();
            ...
            if (Console.ReadLine()?.ToLower() == "y")
            {
                documentDb.SavePage(result);  
                Console.WriteLine("Сохранено в документную БД");
            }

            Console.ReadKey();
        }

        void ShowScrapedPages(ScrapingDb db)  
        {
            Console.Clear();
            Console.WriteLine("Сохраненные страницы (документная БД):\n");

            List<ScrapedPage> pages = db.GetAllPages();
            ...
            Console.WriteLine($"Всего в БД: {db.GetTotalCount()}\n"); 
        ...
        void ShowSettings(SettingsService settingsService)
        {
            while (true)
            {
                ...
                Console.WriteLine($"3.Файл БД GitHub: {settingsService.CurrentSettings.GitHubReposDatabase}");
                Console.WriteLine($"4.Файл БД скраппера: {settingsService.CurrentSettings.ScrapingDatabase}");
                ...
                switch (choice)
                {
                    ...
                    case "3":
                        Console.Write("Имя файла БД GitHub (например: mydb.db):");
                        string? dbGitName = Console.ReadLine();
                        if (!string.IsNullOrWhiteSpace(dbGitName))
                        {
                            settingsService.UpdateSettings(s => s.GitHubReposDatabase = dbGitName);
                        }
                        break;

                    case "4":
                        Console.Write("Имя файла БД скрапера (например: scraper.db):");
                        string? dbScraperName = Console.ReadLine();
                        if (!string.IsNullOrWhiteSpace(dbScraperName))
                        {
                            settingsService.UpdateSettings(s => s.ScrapingDatabase = dbScraperName); 
                        }
                        break;
                    ...
                    case "5":
                        ...
                        {
                            s.GitHubReposDatabase = "gitHubRepos.db";
                            s.ScrapingDatabase = "Scraper.db";
                            s.CountPage = 10;
                            s.Timeout = 30;
                        });
